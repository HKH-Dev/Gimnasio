...
public class GeneradorReportesTest {
    ...
    void toptresUsuariosMasActivos() {
        ...
        // top 3 clientes con mas calorias quemadas
        ArrayList<RegistroEntrenamiento> topEsperado = new ArrayList<>();
        topEsperado.add(registro9);  // 2200 calorias quemadas
        topEsperado.add(registro5);  // 2100 calorias quemadas
        topEsperado.add(registro14); // 2000 calorias quemadas

        GeneradorReportes generadorReportes = new GeneradorReportes();
        ArrayList<RegistroEntrenamiento> topTest = generadorReportes.toptresUsuariosMasActivos(registros);

        Assertions.assertEquals(topEsperado.size(), topTest.size(), "Top 3 tamanio listas deberia ser igual");
        for (int i = 0; i < topEsperado.size(); i++) {
            Assertions.assertEquals(topEsperado.get(i).getCliente().getCedula(), topTest.get(i).getCliente().getCedula(),
                    "EL cliente de la posicion " + i + " deberia coincidir con el resultado esperado");
        }
    }
